...
def create_dataset(stock_symbol, start_date, end_date, lags=5):
    ...
    # we want ro use additional features: lagged returns ... today's returns, yesterday's returns etc.
    tslag = pd.DataFrame(index=df.index)
    tslag['Today'] = df['Adj Close']
    tslag['Volume'] = df['Volume']

    # create the classified lag series of prior trading period close values
...
if __name__ == "__main__":
    ...
    model = KNeighborsClassifier(285)
    ...
    # output the hit-race and the confusion matrix for the model
    print("Accuracy of kNN model", model.score(X_test, Y_test))
