...
risk_free_rate = float(input('Enter risk free rate here'))
...
def capm(start_date, end_date, ticker1, ticker2):
    ...
    return_stock2 = stock2.resample('D').last()
    ...
    data[['s_returns', 'm_returns']] = np.log(data[['s_adjclose', 'm_adjclose']]/data[['s_adjclose', 'm_adjclose']].
                                              shift(1))
    # no need for NaN/missing values so let's get rid of them
    data = data.dropna()

    # covariance matrix: the diagonal items are variances -off diagonals are covariances
    ...
    plt.xlabel('Market returns $R_a$', fontsize=18)
    ...
    expected_return = risk_free_rate + beta*(data['m_returns'].mean()*252-risk_free_rate)
...
if __name__ == "__main__":
    ...
    start_date = input('Enter start date here')
    end_date = input('Enter end date here')
    capm(start_date, end_date, sticker, '^GSPC')
